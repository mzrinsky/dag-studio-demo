# 🔌 Link Routing Specification

## 1. Architectural Philosophy: The "View-Only" Path
The global state store tracks links as a simple mapping: `Link { sourcePortId, targetPortId }`. It does **not** store the SVG path data. 

The `ConnectionCanvas` is responsible for calculating the path in real-time based on the current spatial coordinates of the Ports. This ensures that as modules move, the lines update fluidly without needing to commit "path updates" to the global state engine.
...
*   **State Storage**: To keep the core graph logic clean, waypoint coordinates are **not** stored in the primary `links` array. Instead, they are persisted in a `shallow` `linkMetadata` object within the `useGraphStore`, keyed by the link's unique ID.
...
1.  **Administrative Enforcement (Highest Priority)**: 
    *   **Scope**: Graph-wide or Link-specific.
...
2.  **User-Specific Override (Medium Priority)**:
    *   **Per-Route**: A user manually changes the style of a single link.
...
### A. Patch Validation (Law of Acyclicity)
...
*   **Bidirectional Warning**: Simultaneously, the source Port from which the plug originated should receive a "Warning" visual state to notify the user that the current patch attempt is invalid.
*   **Invalid Drop**: Any attempt to commit a link that violates graph laws is silently rejected by the store, and the plug snaps back to the origin port.

### B. Proximity & Short-Circuiting
*   **Short-Route Threshold**: A global distance threshold (e.g., 50px) is defined. 
*   **Universal Fallback**: If the distance between ports falls below this threshold, OR if a complex routing algorithm (A*) exceeds its computational budget, the system automatically forces the **Linear** composite strategy (Straight Line vs. Cable based on the distance logic in Section 2.A). This ensures that link visibility is never sacrificed for aesthetic routing.

### C. The Active Plug (Ghost Path)
While a user is dragging a new patch lead from a port but has not yet dropped it, the routing defaults to the **"Cable" (Catenary)** style. This provides a tactile, fluid feel during the patching process, regardless of the final routing preference.

### D. Anchor Point Offsets
*   **Anchor Point Offsets**: Routing calculations must not use the center coordinates of the Port element. Instead, they must use the **Anchor Point** (the specific edge/socket of the port). This ensures lines originate from the boundary of the module, preventing visual overlap with the module's internal content.
*   **Orthogonal Fallback**: In the event that the Smart Routing (A*) cannot find a valid path that avoids module occlusion, the system will fallback to a **Standard Manhattan Route** (90-degree angles, ignoring collisions) to ensure the link is always visible.

## 5. Performance & Dynamic Degradation
Since recalculating 100+ Bezier curves or A* paths every frame during a module drag is expensive, the system employs **Dynamic Degradation**:

1.  **Drag-State Fallback**: When any module involved in a link is in an `active-drag` state, the Routing Engine temporarily disables "Smart Routing" (A*) and "Manual" waypoints, falling back to **Organic Cubic Bezier** or **Linear** paths.
2.  **Re-Calculation Trigger**: The expensive A* pathfinding is only re-triggered `onDragEnd`.
3.  **Memoized Coordinates**: Use a selector to only trigger re-renders of lines connected to the module currently being moved.
4.  **SVG Grouping**: Wrap all links in a single `<g>` element to minimize DOM overhead.
...
*   **Content**: All standard, inactive links.
*   **Z-Index Logic**: This layer sits behind all `<Module>` components.
*   **Behavior**: Purely passive. It renders the bulk of the graph's wiring.

### B. The Module Layer (Midground)
*   **Content**: The `<Module>` components and their internal `<Ports>`.
*   **Z-Index Logic**: Managed by the `useGraphStore` (Bring to Front).

### C. The Interaction Overlay (Foreground)
*   **Content**: Selected links, hovered paths, and the "Active Plug" (Ghost Path).
*   **Z-Index Logic**: This is a transparent SVG overlay that sits on top of the entire canvas.
*   **Dynamic Promotion**: When a link is hovered or selected, it is **not** moved in the DOM. Instead:
...
*   **Event Delegation**: To prevent "phantom clicks," all mirrored paths must carry a `data-link-id` attribute. The Interaction Overlay implements a single event listener that intercepts interactions and maps the `linkId` back to the global state engine, ensuring that the logical link is targeted regardless of which visual layer was touched.
...
## 7. Link Z-Index Inheritance
To ensure visual consistency, the "depth" of a link's influence is derived from its endpoints:

1.  **Depth Calculation**: A link's implicit Z-index is $\max(\text{sourceModule.zIndex}, \text{targetModule.zIndex})$.
2.  **Visual Parity**: If a module is brought to the front, the links attached to it do not necessarily move to the foreground (unless selected), but they are logically grouped with that module's spatial priority for the purpose of occlusion calculations in Orthogonal routing.
