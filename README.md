...
DAG Studio moves beyond simple module-graph visualization. It is a specialized, high-performance framework designed to model, visualize, and execute complex, multi-stage data transformations using a highly decoupled architecture.
...
This project started from a simple desire to build a fun, "patch cable" style interface. Given that software like *Reason* has existed for nearly three decades, I assumed this was a solved problem. However, after searching for libraries that fit the bill, I found that most available solutions focus heavily on the visual "modules and links" rather than the underlying data contract.
...
While traditional node libraries (like ReactFlow) treat links as simple lines between boxes, DAG Studio treats the graph as a *defined computational blueprint*. By centering the architecture on **Data Ports**, we introduce a strict "Contract" for every module. A port isn't just a socket; it's a typed interface that manages a **Stability Gradient**—allowing data to evolve through the **Data Quad** from a volatile draft to a verified result and finally to a persisted record.

### 📖 A Note on Terminology
To avoid confusion with generic graph theory or React components, DAG Studio adopts a **Module-centric** lexicon. While these concepts map to "Nodes" and "Edges" in other systems, we use specific terms to reinforce our decoupling philosophy:
*   **Module**: The primary unit of logic and interface (What others call a "Node").
*   **Shell**: The visual React container that hosts a Module's interface.
*   **Patch/Link**: The connection between two Ports (What others call an "Edge").
*   **Port**: The typed contract through which data enters or leaves a Module.
...
*   **`<DAGFlow>`**: The root context provider. Manages the global coordinate system (D3) and provides the canvas for rendering modules and links.
*   **`<LinkCanvas>`**: A pure view layer that subscribes to the global store to render SVG links between ports.
*   **`<Module>` (Module Shell)**: The "dumb" container. Responsible *only* for layout and visual structure. It holds no business logic or flow state.
...
*   **`Port` & `Plug`**: The interactive interface. A **Port** is the static connection point; a **Plug** is the active draggable entity created when a user initiates a patch from a Port.
...
*   **UUID Mandate**: Every Module and Port *must* have a stable, unique ID. Array indices are strictly forbidden as keys.
*   **Global Registry**: All positions, z-indices, and link mappings are stored in a global store (`useGraphStore`). **The Store is the sole Source of Truth and the Orchestrator of all data flow**; it decides when to trigger port callbacks based on graph dependencies.
*   **State Partitioning**: 
...
*   **[Link Routing](docs/drafts/CONNECTION_ROUTING.md)**: Logic for path generation (Linear, Organic, Orthogonal), the Layered Overlay Model, and dynamic performance degradation.
...
| `type` | `PortType` | Strict type descriptor (Primitives or Custom IDs) used for patch validation. |
...
1.  **Law of Decoupling**: Business logic *never* lives in `<Module>`. It belongs in the `Ports` declaration, the store actions, or the wrapped worker component.
...
DAG Studio stands on the shoulders of giants. The architectural philosophy of this project is heavily inspired by the elegance of immediate-mode GUIs and the robustness of professional module-based editors:
